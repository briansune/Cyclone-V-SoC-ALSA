...
static int opencores_i2s_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	if (val == 1) {
		...
		// Example safe default dividers for typical 48kHz sample rate:
		// mclk_div = 0; bclk_div = 3; lrclk_div = 15 (playback and capture)
		regmap_update_bits(i2s->regmap_clk, CLK_CTRL1,
			MCLK_DIV_MASK | BCLK_DIV_MASK | CLK_MASTER_SLAVE,
			(0 << MCLK_DIV_SHIFT) | (3 << BCLK_DIV_SHIFT) | CLK_MASTER_SLAVE);

		regmap_update_bits(i2s->regmap_clk, CLK_CTRL2,
			PB_LRC_DIV_MASK | CAP_LRC_DIV_MASK,
			(15 << PB_LRC_DIV_SHIFT) | (15 << CAP_LRC_DIV_SHIFT));
	}

	// Clear FIFOs after format and mode change
...
/* New version for kernel 6.12+ */
static int opencores_i2s_component_probe(struct snd_soc_component *component)
{
    struct opencores_i2s *i2s = snd_soc_component_get_drvdata(component);
    struct snd_soc_dai *dai;

    dev_dbg(component->dev, "component_probe\n");

    /* Iterate all DAIs owned by this component to init DMA data */
...
static const struct snd_soc_component_driver opencores_i2s_component = {
	.name = "opencores-i2s",
	.probe = opencores_i2s_component_probe,
};
